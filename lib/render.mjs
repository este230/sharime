...
const X264_ARGS = ['-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20']
const AMF_ARGS = [
  '-c:v', 'h264_amf',
  '-usage', 'transcoding',
  '-quality', 'speed',
...
let encoderArgsPromise = null

async function amfAvailable() {
  const probe = join(tmpdir(), `${makeId('amf')}.mp4`)
  const { code } = await run('ffmpeg', [
    '-hide_banner', '-v', 'error', '-y',
    '-f', 'lavfi', '-i', 'testsrc2=size=320x180:rate=30:duration=0.2',
    '-frames:v', '3', '-vf', 'format=yuv420p',
    ...AMF_ARGS, '-pix_fmt', 'yuv420p',
    probe,
  ])
...
async function videoEncoderArgs() {
  encoderArgsPromise ||= amfAvailable().then((ok) => (ok ? AMF_ARGS : X264_ARGS))
  return encoderArgsPromise
}
...
async function compositeBehind(seg, canvas, segmentsDir, hash, outFile, onMatte) {
  ...
  try {
    ...
    await ffmpeg([
      ...
      ...(await videoEncoderArgs()), '-pix_fmt', 'yuv420p',
      outFile,
    ])
  } finally {
...
function xfadeName(transition) {
  if (transition === 'dip-to-black') return 'fadeblack'
  if (transition === 'wipe-left') return 'wipeleft'
  if (transition === 'wipe-right') return 'wiperight'
  return 'fade'
}

function transitionDur(seg) {
  return Math.max(0.08, Math.min(0.75, Number(seg?.transitionDur) || 0.3))
}

async function concatXfade(segFiles, edl, canvas, workDir) {
  const inputs = []
  ...
  for (let k = 1; k < segFiles.length; k++) {
    ...
    if (edl[k]?.transition === 'cut' || !(edl[k]?.transitionDur > 0)) {
      filters.push(`${prevLabel}[${k}:v]concat=n=2:v=1:a=0${outLabel}`)
      accLen += durs[k]
      prevLabel = outLabel
      continue
    }
    const d = Math.min(transitionDur(edl[k]), Math.max(0.08, durs[k - 1] / 2), Math.max(0.08, durs[k] / 2))
    const T = xfadeName(edl[k]?.transition)
...
function musicRegion(music, videoDur) {
  const raw = music && typeof music === 'object' ? music : {}
  const duration = Math.max(0, Number(raw.duration) || 0)
  const start = Math.max(0, Math.min(Number(raw.start) || 0, Math.max(0, duration - 0.1)))
  const endRaw = Number(raw.end) || 0
  const end = endRaw > start + 0.1 ? Math.min(endRaw, duration || endRaw) : 0
  const span = end ? end - start : Math.max(0.1, duration - start)
  return { start, end, span: Math.max(0.1, Math.min(span || videoDur, videoDur || span || 0.1)) }
}

async function muxMusic(videoFile, musicFile, outFile, videoDur, music = null) {
  const fadeOutStart = Math.max(0, videoDur - 1.4)
  const region = musicRegion(music, videoDur)
  const trim = [`atrim=start=${region.start.toFixed(3)}`]
  if (region.end) trim[0] += `:end=${region.end.toFixed(3)}`
  trim.push('asetpts=PTS-STARTPTS')
  trim.push('aloop=loop=-1:size=2147483647')
  trim.push('afade=t=in:st=0:d=0.5')
  trim.push(`afade=t=out:st=${fadeOutStart.toFixed(2)}:d=1.4`)
  await ffmpeg([
    '-y',
    '-i', videoFile,
    '-i', musicFile,
    '-t', String(videoDur),
    '-filter_complex', `[1:a]${trim.join(',')}[a]`,
    '-map', '0:v:0', '-map', '[a]',
...
// ctx: { canvas:{w,h,fps}, edl, clipsById:{id->absPath}, musicFile, music, segmentsDir, workDir, outFile, adjust }
export async function render(ctx, { onProgress = () => {} } = {}) {
  const { canvas, edl, clipsById, musicFile, music, segmentsDir, workDir, outFile, adjust } = ctx
  ...
  const joined = canXfade
    ? await concatXfade(segFiles, edl, canvas, workDir)
  ...
  await muxMusic(joined, musicFile, outFile, videoDur, music)
