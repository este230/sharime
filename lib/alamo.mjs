...
export function deterministicRecipe(fingerprint = {}, _opts = {}) {
  ...
  const beatsPerCut = energy === 'high' ? 1.5 : energy === 'low' ? 4 : 2
  const moodToGrade = {
    warm: 'warm', cool: 'cool-cinematic', moody: 'moody',
    vibrant: 'vibrant', muted: 'muted-cool', natural: 'natural',
  }
  const grade = moodToGrade[fingerprint.colorMood] || 'muted-cool'
  return {
    grade,
    beatsPerCut,
    transition: energy === 'low' ? 'crossfade' : 'cut',
    transitionDur: energy === 'low' ? 0.4 : 0,
    speed: energy === 'low' ? 'slight-slowmo' : 'dynamic',
    introWide: true,
    textStyle: 'none',
    vibe: `${energy}-energy ${grade} travel cut`,
    notes: 'Built from the reference measurements (ALAMO offline).',
    source: 'built-in',
  }
}
