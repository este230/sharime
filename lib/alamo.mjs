...
function coerceRecipe(obj, fallback) {
  ...
  return {
    ...
    textStyle: obj.textStyle === 'minimal-location' ? 'minimal-location' : fallback.textStyle || 'none',
...
function noteHas(notes, words) {
  const s = String(notes || '').toLowerCase()
  return words.some((w) => s.includes(w))
}

// Phone review notes are the closest thing Sharimie has to "she actually changed
// this." Keep tuning conservative and reversible: only nudge broad defaults that
// match common watch-through feedback, then leave detailed fixes in Review.
export function tuneRecipeForReviewFeedback(recipe, notes) {
  if (!notes || !String(notes).trim()) return recipe
  const tuned = { ...recipe }
  const fast = noteHas(notes, ['too fast', 'rushed', 'frantic', 'jumpy', 'hard to follow', 'hard to read'])
  const slow = noteHas(notes, ['too slow', 'dragged', 'dragging', 'boring', 'too long', 'snappier'])
  const warm = noteHas(notes, ['warmer', 'too cool', 'more warm', 'golden', 'sunset'])
  const cool = noteHas(notes, ['cooler', 'too warm', 'less orange', 'less yellow', 'more cinematic'])
  const soft = noteHas(notes, ['smoother', 'softer', 'less abrupt', 'less harsh', 'fade'])
  const crisp = noteHas(notes, ['hard cut', 'clean cut', 'less fade', 'no fade', 'punchier'])
  const titleLow = noteHas(notes, ['title too low', 'titles too low', 'text too low', 'phone controls', 'caption too low'])

  if (fast) {
    tuned.beatsPerCut = Math.max(Number(tuned.beatsPerCut) || 1.5, 2)
    if (tuned.speed === 'dynamic') tuned.speed = 'slight-slowmo'
  }
  if (slow) {
    tuned.beatsPerCut = Math.min(Number(tuned.beatsPerCut) || 2, 1.5)
    tuned.speed = 'dynamic'
  }
  if (warm) tuned.grade = tuned.grade === 'muted-cool' || tuned.grade === 'cool-cinematic' ? 'golden-hour' : 'warm'
  if (cool) tuned.grade = tuned.grade === 'warm' || tuned.grade === 'golden-hour' ? 'muted-cool' : 'cool-cinematic'
  if (soft && !crisp) {
    tuned.transition = 'crossfade'
    tuned.transitionDur = Math.max(Number(tuned.transitionDur) || 0, 0.25)
  }
  if (crisp && !soft) {
    tuned.transition = 'cut'
    tuned.transitionDur = 0
  }
  if (titleLow) {
    tuned.textStyle = tuned.textStyle === 'none' ? 'minimal-location' : tuned.textStyle
    tuned.textY = 0.68
  }
  tuned.notes = `${tuned.notes || ''} Tuned from phone review feedback.`.trim().slice(0, 300)
  return tuned
}

// Pure, fingerprint-derived recipe. Always available.
export function deterministicRecipe(fingerprint = {}, opts = {}) {
  ...
  return tuneRecipeForReviewFeedback({
    grade,
    beatsPerCut,
    transition,
...
export function houseRecipe(opts = {}) {
  return tuneRecipeForReviewFeedback({
    grade: 'muted-cool',
    beatsPerCut: 1.5,
    transition: 'cut',
...
export async function requestRecipe({ fingerprint, referenceFile, notes, clipCount, musicBpm, musicSeconds, preset }, config = loadConfig()) {
  const fallback = deterministicRecipe(fingerprint, { notes })
  const key = readGatewayKey(config)
  ...
  if (referenceFile) {
    try {
      ...
      if (frames.length) {
        ...
        return tuneRecipeForReviewFeedback(coerceRecipe(extractJson(reply), fallback), notes)
      }
    } catch {
